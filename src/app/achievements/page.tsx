...
export default function AchievementsPage() {
  ...
  const mockAchievements: Achievement[] = [
    ...
    },
    {
      ...
      participationType: 'Individual',
      timeWindow: { startDate: '', endDate: '' },
      rewards: [{ name: 'Gamerscore', description: 'Points', value: '50', type: 'Gamerscore', valueType: 'Int' }],
    ...
    },
    {
      ...
      titleAssociations: [{ name: 'Microsoft Flight Simulator', id: 1234567890 }],
      ...
      participationType: 'Individual',
      timeWindow: { startDate: '', endDate: '' },
      rewards: [{ name: 'Gamerscore', description: 'Points', value: '30', type: 'Gamerscore', valueType: 'Int' }],
    ...
    },
    {
      ...
      participationType: 'Individual',
      timeWindow: { startDate: '', endDate: '' },
      rewards: [{ name: 'Gamerscore', description: 'Points', value: '100', type: 'Gamerscore', valueType: 'Int' }],
    ...
    },
    {
      ...
      participationType: 'Individual',
      timeWindow: { startDate: '', endDate: '' },
      rewards: [{ name: 'Gamerscore', description: 'Points', value: '75', type: 'Gamerscore', valueType: 'Int' }],
  ...
  const handleSearch = async (query: string) => {
    ...
    try {
      ...
      if (response.success) {
        const filtered = response.data.filter(achievement =>
        ...
        const filtered = mockAchievements.filter(achievement =>
          achievement.name.toLowerCase().includes(query.toLowerCase()) ||
          achievement.description.toLowerCase().includes(query.toLowerCase()) ||
          achievement.titleAssociations.some(title =>
            title.name.toLowerCase().includes(query.toLowerCase())
  ...
  const handleFilterChange = (filter: string) => {
    ...
    switch (filter) {
      case 'achieved':
        filtered = mockAchievements.filter(a => a.progression.achievementState === 'Achieved');
        break;
      case 'inprogress':
        filtered = mockAchievements.filter(a => a.progression.achievementState === 'InProgress');
        break;
      case 'notstarted':
        filtered = mockAchievements.filter(a => a.progression.achievementState === 'NotStarted');
        break;
      case 'secret':
  ...
  const displayedAchievements = searchQuery ? achievements : (selectedFilter ? achievements : mockAchievements);
  
  const totalGamerscore = displayedAchievements.reduce((sum, achievement) => {
    const gamerscore = achievement.rewards?.find(r => r.type === 'Gamerscore');
    return sum + parseInt(gamerscore?.value || '0');
  }, 0);

  const unlockedCount = displayedAchievements.filter(a => 
    a.progression.achievementState === 'Achieved'
  ).length;
  ...
  return (
    <div className="container py-8">
      ...
      <section className="mb-8">
        <div className="grid grid-3 gap-4 mb-6">
          <div className="card text-center">
            <div className="text-2xl font-bold text-primary">{displayedAchievements.length}</div>
            <div className="text-muted text-sm">Total de Conquistas</div>
          </div>
          <div className="card text-center">
            <div className="text-2xl font-bold text-success">{unlockedCount}</div>
            <div className="text-muted text-sm">Desbloqueadas</div>
          </div>
          <div className="card text-center">
            <div className="text-2xl font-bold text-warning">{totalGamerscore.toLocaleString()}G</div>
      ...
      {!searchQuery && !selectedFilter && (
        <section className="mb-12">
          <h2 className="text-2xl font-bold mb-6 text-center">Tipos de Conquistas</h2>
      ...
      {!searchQuery && !selectedFilter && (
        <section className="mb-12">
          <h2 className="text-2xl font-bold mb-6 text-center">Progresso Geral</h2>
          <div className="card max-w-2xl mx-auto">
            <div className="mb-4">
              <div className="flex justify-between text-sm mb-2">
                <span>Conquistas Desbloqueadas</span>
                <span>{unlockedCount} / {mockAchievements.length}</span>
              </div>
              <div className="progress">
                <div 
                  className="progress-bar" 
                  style={{ width: `${(unlockedCount / mockAchievements.length) * 100}%` }}
                />
              </div>
            </div>
      ...
      <section className="text-center">
        <div className="card max-w-2xl mx-auto">
          ...
          <div className="text-left text-muted space-y-2">
            <p>• <strong>Desbloqueadas:</strong> Conquistas já completadas pelo jogador</p>
            <p>• <strong>Em Progresso:</strong> Conquistas com progresso parcial</p>
            <p>• <strong>Não Iniciadas:</strong> Conquistas ainda não começadas</p>
            <p>• <strong>Secretas:</strong> Conquistas com critérios ocultos</p>
            <p>• <strong>Gamerscore:</strong> Pontos obtidos por conquistar achievements</p>
          </div>
        </div>
      </section>
    </div>
